(function () {
        ...
        setTimeout(async () => {
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error("Traversal timeout")), 5000); // 5 second timeout
                });
    ...
    // Helper function to safely serialize data with depth limiting
    function safeSerialize(obj, depth = 0, maxDepth = 3) {
        ...
        if (Array.isArray(obj)) {
            ...
            const maxItems = Math.min(obj.length, 5); // Further reduced array size
            
            for (let i = 0; i < maxItems; i++) {
                serializedArray.push(safeSerialize(obj[i], depth + 1, maxDepth));
            }
            
            if (obj.length > maxItems) {
                serializedArray.push(`[... ${obj.length - maxItems} more items]`);
            }
    ...
    // Keep the synchronous version as fallback (unused now but might be useful)
